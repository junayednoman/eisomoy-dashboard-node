...
const AddGallery = () => {
    ...
    const handleAddgallery = async (values: any, { resetForm }: any) => {
        ...
        if (values.gallery_images && values.gallery_images.length > 0) {
            const formData = new FormData();
            for (let i = 0; i < values.gallery_images.length; i++) {
                formData.append('files', values.gallery_images[i]);
            }
        
            try {
                const response = await axios.post('/api/multiuploads', formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                });
                ...
                // Convert the array of filenames into a comma-separated single string
                galleryImageNames = response.data.filenames.join(',');
            } catch (error) {
        ...
        const formDataFinal = {
            ...
            gallery_Names: galleryImageNames,
            featured_image: featuredImageName,
            category: categoriesString,
            uploader_name: values.reporter_name,
        ...
        try {
            const response = await axios.post(`${apiUrl}/api/gallery/add-news`, formDataFinal, { withCredentials: true });
            console.log('gallery added succesfully');
            Swal.fire({
                icon: 'success',
            ...
            Swal.fire({
                ...
                text: error.response?.data?.message || 'Failed to add Gallery',
                timer: 3000,
    ...
    return (
        <>
            <h4 className="text-2xl font-semibold mb-8">Add a Gallery</h4>
            <Formik
                initialValues={{
                    title: '',
                    gallery_images: null,
                ...
                onSubmit={(values, { resetForm }) => handleAddgallery(values, { resetForm })}
                        ...
                        <div className="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-3 gap-x-6">
                            <div className="xl:col-span-3 lg:col-span-2 md:col-span-2">
                                <div>
                                    ...
                                    <Field name="title" type="text" id="title" placeholder="Enter News Title" className="form-input h-12" />
                                    {errors.title && touched.title && <p className="text-red-500">{errors.title}</p>}

                                    <label htmlFor="gallery_images" className="mt-4">Description</label>
                                    <input name="gallery_images" type="file" id="gallery_images" className="form-input h-12 mb-2" onChange={(event: any) => { setFieldValue('featured_images', event.currentTarget.files); }} multiple />
                                    ...
                                    <div className="rounded-t-md bg-white dark:bg-black dark:myAccordian">
                                        ...
                                        <AnimateHeight duration={50} height={active === 1 ? 'auto' : 0}>
                                            <div className="p-4 pt-2 font-semibold text-white-dark">
                                                <div className="mb-3">
                                                    <label htmlFor="uploader_name">Reporter Name</label>
                                                    <Field name="uploader_name" type="text" id="uploader_name" placeholder="Enter uploader name" className="form-input h-12 mb-2" />
                                                </div>
                                                <div className="mb-3">
...
export default withAuth(AddGallery);
